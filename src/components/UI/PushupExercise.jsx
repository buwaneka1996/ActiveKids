...
const PushupExercise = ({ onClose }) => {
    ...
    const detectPushup = useCallback((pose) => {
        ...
        if (leftShoulder && rightShoulder && leftElbow && rightElbow && leftWrist && rightWrist) {
            ...
            const isDown = leftArmAngle < 90 && rightArmAngle < 90;
            const isUp = leftArmAngle > 150 && rightArmAngle > 150;

            setProgress((leftArmAngle + rightArmAngle) / 2);

            //console.log(`Left Arm Angle: ${leftArmAngle.toFixed(2)}, Right Arm Angle: ${rightArmAngle.toFixed(2)}`);
            //console.log(`Pose State: ${poseState}, Stable Frames: ${stableFramesRef.current}`);
            ...
            console.error("keypoints not detected properly");
    ...
    useEffect(() => {
        const loadPosenet = async () => {
            ...
            const detectorRef = await poseDetection.createDetector(poseDetection.SupportedModels.MoveNet, {
            ...
            const detectPose = async () => {
                if (webcamRef.current && webcamRef.current.video.readyState === 4) {
                    ...
                    const pose = await detectorRef.estimatePoses(video);
